...
export class CartComponent {
  ...
  updateQuantity(item: CartItem, increment: boolean) {
    ...
    if (index !== -1) {
      const newQuantity = increment ? items[index].quantity + 1 : Math.max(0, items[index].quantity - 1);
  ...
  checkout() {
    ...
    console.log('Delivery mode:', this.isDelivery());
    console.log('Selected table:', this.selectedTable);
    if (this.isDelivery()) {
      console.log('Delivery person:', this.deliveryPerson);
    }
